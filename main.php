<?php
		...
		// create databases 'sql' and two tables 'dbs' and 'tabs'
		$conn = mysqli_connect("localhost","root","");
		$command="CREATE DATABASE sqlDB";
		$query = mysqli_query($conn,$command);
		mysqli_close($conn);
		
		$conn = mysqli_connect("localhost","root","","sqlDB");
		$command="CREATE TABLE dbs (
				db varchar(255)
				);";
				
		$query = mysqli_query($conn,$command);
		$command="CREATE TABLE tabs (
				tab varchar(255)
				);";				
		$query = mysqli_query($conn,$command);
		 ...
		 if($error_messages == ""){
			...
			if(! $query ) {
				...
				if(!strpos($command,"DATABASE") && !strpos($command,"TABLE")){
					...
					echo"<table id=".strval("table")." border=1 >";
					foreach($rows as $row){
			...
			if(strpos($command,"DROP")!==false && strpos($command,"DATABASE")!==false){
				...
				$conn2=mysqli_connect("localhost","root","","sqlDB");
			...
			if(! $query2 ) {
				...
				print("Command executed suceussfully");
			}
